...
@Injectable()
export abstract class AbstractStaticService implements OnModuleDestroy {
  protected readonly logger: Logger;
  protected readonly proxy: ProxyServer;
  protected readonly targetServerUrl: string;

  constructor(
    protected readonly appConfigService: AppConfigService,
    protected readonly metricsService: MetricsService,
    protected readonly httpAgent: http.Agent,
    protected readonly httpsAgent: https.Agent,
    protected readonly serviceName: string,
    protected readonly inspectService: InspectService,
  ) {
    ...
    this.logger.log(
      `${serviceName} initialized with target server: ${this.targetServerUrl}`,
    );
  }
  ...
  private setupProxyListeners(proxy: ProxyServer, serverIndex: number): void {
    ...
    proxy.on("error", (err, _req, res: http.ServerResponse) => {
      ...
      if (!res.headersSent) {
        ...
        res.end(
          JSON.stringify({
            message: `Internal ${this.serviceName} error (server ${serverIndex})`,
          }),
        );
      }
    });
